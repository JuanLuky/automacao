...
- **Ação manual pendente do usuário**: habilitei `MESSAGES_EDITED`/`MESSAGES_DELETE` no webhook da instância via `POST /webhook/set/atendimento-empresa` (webhook é configurado direto na Evolution API, não no repo — ver CLAUDE.md). Falta **reimportar `fluxo-completo-com-backend.json` manualmente na UI do n8n** (mesma convenção de sempre — `git push`/edição do JSON no repo não afeta o workflow rodando) pra esses dois novos ramos existirem de fato no n8n. Não testado ainda com edição/apagamento real do lado do cliente.
