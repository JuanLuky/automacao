...
- **Endpoint novo**: `GET /contacts/whatsapp/avatar?instance=&numero=` (`ContactsController`, `JwtAuthGuard` — mesmo nível de acesso das outras rotas de `/contacts`, abertas a todos). Independente de conversa de propósito: o remetente de uma mensagem de grupo (`Message.remetente_telefone`) não tem uma conversa própria, só o telefone salvo na mensagem — reaproveita `EvolutionService.getProfilePictureUrl` já existente, só expõe num endpoint que aceita qualquer número.
...
**Testado só o endpoint via curl nesta sessão** (não o fluxo completo com uma mensagem de grupo real recebida depois desta mudança) — validar visualmente no navegador antes de considerar concluído, junto com o resto dos itens pendentes desta sessão.
...
- [x] Avatar de quem escreveu cada mensagem dentro de um grupo (`GET /contacts/whatsapp/avatar?numero=`, cacheado por telefone client-side na tela de chat) (2026-08-17). Ver "Avatar de quem escreveu cada mensagem" acima. `tsc`/`build` limpos nos dois lados; endpoint testado via curl. **Não testado visualmente no navegador nesta sessão.**
