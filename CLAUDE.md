...
Sessão de 2026-08-17 (a mais longa até aqui): remetente em mensagem de grupo + mensagens enviadas direto do celular + envio de áudio pelo painel; gravação de áudio por microfone; aba de Contatos; papel Supervisor + toggle "ver todos os setores"; header com dropdown "Administração"; avatares (foto de perfil) + nome de grupo ao vivo do WhatsApp. **Todo o código concluído, `tsc`/`build` limpos nos dois lados o tempo todo.** Ver "Status atual do projeto" acima pra cada item e sua seção própria mais abaixo.
