...
- **Testado via curl nesta sessão** (login admin, `POST /status`, `GET /status/atual`/`historico`, guard 401 sem token) e via `npm run build`/`tsc --noEmit` do frontend, ambos limpos. **Não testado visualmente no navegador ainda** — ver "Próximos passos".
...
- **Pendente**: JSON atualizado e validado (sintaxe + conexões revisadas), mas **ainda não reimportado no n8n rodando** nesta sessão — decisão deliberada, pra não arriscar o reimport completo derrubar as credenciais de Redis/SMTP já configuradas manualmente nos nodes de debounce/healthcheck (credencial não vem no export, ver "Alerta de desconexão do WhatsApp"). Os 2 nodes novos não usam credencial nativa do n8n (mesmo padrão `apikey` no header dos outros nodes HTTP), então não têm esse risco — mas o resto do workflow tem. Ver "Próximos passos".
...
- [x] Página de status/uptime — `/status` (pública, manual) + `/status/publicar` (admin) (2026-08-04). Testado via curl (POST/GET com guard admin) e build/tsc do frontend limpos. **Não testado visualmente no navegador ainda.** Ver "Página de status/uptime" acima.
- [x] Horário de funcionamento configurável + resposta automática fora do expediente no WhatsApp (2026-08-04). Backend (`/business-hours`) testado via curl (GET público com `aberto` calculado, PATCH admin). Frontend (`/horario-funcionamento`) com build/tsc limpos. n8n: workflow JSON atualizado e validado (sintaxe + conexões), **mas ainda não reimportado no n8n rodando nem testado com mensagem real** — decisão deliberada nesta sessão pra não arriscar derrubar credenciais de Redis/SMTP já configuradas no reimport completo. Ver "Horário de funcionamento configurável" acima.
...
Itens das sessões de 2026-07-24/27/29/30/31 (editar/inativar/excluir/busca em `/usuarios`, debounce de fragmentos testado com WhatsApp real, healthcheck de desconexão com alerta por e-mail, tela de QR Code do WhatsApp, migrations substituindo `TYPEORM_SYNCHRONIZE`, CRUD de setores, busca de conversas finalizadas, setor novo no menu do WhatsApp via `n8n-novo-setor.md`, respostas rápidas + mensagens automáticas ao Assumir/Finalizar, paginação na fila) **já concluídos** — ver "Status atual do projeto" acima.

Pendências imediatas da sessão de 2026-08-04 (página de status + horário de funcionamento):

- **Reimportar `fluxo-completo-com-backend.json` no n8n rodando** e testar com uma mensagem real (ou curl no webhook) mudando o horário pra "fechado" — ver "Horário de funcionamento configurável" acima pro motivo de não ter sido feito na própria sessão (risco de derrubar credenciais de Redis/SMTP já configuradas no reimport completo).
- **Testar `/status` e `/status/publicar` visualmente no navegador** — só validado via curl/build nesta sessão, sem checagem manual na UI ainda.
...
- **Anexos/mídia (áudio, imagem, PDF) do WhatsApp** — levantado pelo usuário em 2026-08-04 como próximo passo depois das features de status/horário, **ainda não implementado**: hoje o sistema só trata texto (`Extrair Dados da Mensagem` no n8n só lê `data.message.conversation`/`extendedTextMessage`), então uma mídia enviada pelo cliente não aparece no painel — o atendente perde contexto e precisa checar pelo celular físico da instância. Vai exigir decisão de onde armazenar o arquivo (a Evolution API expõe a mídia via download separado, não inline no payload do webhook) e mudança de schema em `Message` (hoje só tem `mensagem: text`).
