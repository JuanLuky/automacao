...
- **Atendente vê só a fila do próprio setor** (`user.departamento_id` do JWT). **Admin vê todas**, com um `Select` de setor (`user.role === 'admin'` libera o filtro). **Supervisor** (papel novo, 2026-08-17) é como atendente por padrão, mas tem um toggle "Ver todos os setores" (componente `Switch`, estado em `useVerTodosSetores`, salvo em `localStorage` — mesmo padrão de `useTheme`) que libera o mesmo comportamento do admin enquanto ligado. `podeVerTodos = isAdmin || (isSupervisor && verTodos)` substitui os antigos checks isolados de `isAdmin` nas páginas de `fila` e `dashboard` (`semSetor`, `filtroDepartamento`, exibição do `Select`/breakdown "por setor"). Essa lógica fica nas páginas de `fila` e `dashboard`, não espalhada pelos componentes.
...
- **"Contatos do WhatsApp"**: busca ao vivo via `EvolutionService.getContacts` (`POST /chat/findContacts/{instance}`), **nunca persistida** — toda visita à tela refaz a chamada (botão "Atualizar" força um novo fetch manual; sem polling automático, ao contrário de `/whatsapp`, que monitora conexão e por isso justifica polling — contato muda bem menos que status de conexão). Endpoint novo `GET /contacts/whatsapp?instance=` devolve o JSON cru da Evolution API sem normalizar (mesmo padrão de `/whatsapp/status`/`/whatsapp/qrcode` — formato varia entre versões); quem normaliza é `normalizarContatoWhatsapp` em `contatos/page.tsx`. **Testado contra a instância real nesta sessão (2026-08-17)**: o formato real difere do que tinha sido assumido sem teste — o JID vem em `remoteJid`, não em `id` (esse é a chave interna do registro no banco da Evolution API; usá-lo produzia telefone com lixo) — corrigido. Filtra pra só `remoteJid` terminando em `@s.whatsapp.net`; contatos com `@lid` (identificador vinculado, sem número exposto) e grupos (`@g.us`, já cobertos por `/grupos`) são descartados.
...
`tsc --noEmit`/`npm run build` limpos nos dois lados. Migration `AddContacts` rodada contra o banco de dev (2026-08-17). **Ainda não testado visualmente no navegador** (extensão do Chrome não conectada nesta sessão) — o backend/Evolution API já foram validados via curl (ver acima), falta só confirmar a tela em si.

### Papel Supervisor + toggle "ver todos os setores" (2026-08-17)

Terceiro papel de usuário além de `admin`/`atendente` (backend: `UserRole.SUPERVISOR`, migration `AddSupervisorUserRole` — recria o enum Postgres `users_role_enum` em vez de `ALTER TYPE ... ADD VALUE`, mesmo padrão/motivo já usado em `AddVideoMessageTipo`). Comportamento: **como atendente por padrão** (só vê a fila/dashboard do próprio setor), mas ganha um toggle "Ver todos os setores" que, ligado, dá o mesmo alcance do admin (todas as conversas, com o `Select` de setor liberado).

- **Backend não muda nada além do enum** — `GET /conversations` já não filtra por papel/setor (ver "Regras de negócio no frontend" acima), então o supervisor só precisa existir como valor de `role` pra o frontend reconhecer. `CreateUserDto`/`UpdateUserDto` já aceitavam qualquer `UserRole` via `@IsEnum` — nenhuma mudança de DTO foi necessária.
- **Toggle é 100% client-side**: `frontend/src/hooks/useVerTodosSetores.ts`, estado booleano salvo em `localStorage` (chave `atendimento.verTodosSetores`, mesmo padrão hidratação-após-mount de `useTheme`, pra não dar mismatch de SSR). Não é uma permissão validada em lugar nenhum — é uma preferência de exibição, coerente com o resto do modelo de confiança client-side já documentado (admin também não é reforçado no backend pra esse filtro).
- **`Switch`** (`components/ui/Switch.tsx`, novo): toggle genérico (`role="switch"`, sem lib de terceiros), pensado pra ser reaproveitado em qualquer liga/desliga futuro, não só este.
- **`fila`/`dashboard`**: `podeVerTodos = isAdmin || (isSupervisor && verTodos)` substitui os checks que antes eram só `isAdmin` — controla `semSetor`, `filtroDepartamento`, a exibição do `Select` de setor e (no dashboard) o breakdown "por setor". O toggle em si (label + `Switch`) só aparece pra quem é `isSupervisor` — admin nunca precisa dele, já vê tudo sempre.
- **`/usuarios`**: `Select` de papel ganhou a opção "Supervisor"; badge da lista ganhou uma terceira cor (`waiting`, âmbar — distinta do `tide` do admin e do cinza padrão do atendente).

`tsc --noEmit`/`npm run build` limpos nos dois lados. Migration `AddSupervisorUserRole` rodada contra o banco de dev (2026-08-17). **Ainda falta validar visualmente**: criar um usuário supervisor, conferir que sem o toggle ele só vê o próprio setor, e que ligar o toggle libera a visão completa igual a um admin.
...
Pedido do usuário: grupo tem nome e foto no WhatsApp de verdade, queria isso refletido no painel — até aqui grupo sempre aparecia como "Grupo sem nome" (ver "Grupos do WhatsApp" acima, decisão original de não buscar isso pra não gastar uma chamada extra). Também pediu avatar dos contatos (foto de perfil) na fila normal, igual ao WhatsApp.

**Endpoint novo, único pros dois casos**: `GET /conversations/:id/whatsapp-info?instance=` (`ConversationsController.buscarInfoWhatsapp`, `JwtAuthGuard`) — devolve `{ nome, foto_url }`, sempre ao vivo, **nunca persistido** (mesmo espírito de `GET /contacts/whatsapp`, ver "Aba de Contatos" acima). `ConversationsService.buscarInfoWhatsapp` ramifica por `conversa.tipo`:
- **grupo**: `EvolutionService.getGroupInfo` (`GET /group/findGroupInfos/{instance}?groupJid=`, testado contra a instância real desta sessão) devolve `subject` (nome) e `pictureUrl` juntos numa única chamada.
- **cliente**: nome já vem salvo desde a criação da conversa (`cliente_nome`, do `pushName` do primeiro contato) — só busca a foto, via `EvolutionService.getProfilePictureUrl` (`POST /chat/fetchProfilePictureUrl/{instance}`, body `{ number }`), campo `profilePictureUrl`.

Os dois métodos do `EvolutionService` devolvem `null` (em vez de propagar erro) quando a Evolution API responde não-OK — contato/grupo sem foto, ou removido, não pode quebrar a exibição de uma lista inteira por causa de um item.

**Frontend**:
- **`useWhatsappAvatar(conversationId)`** (`hooks/`): busca `{ nome, fotoUrl }` ao montar, falha silenciosa (sem foto/nome, sem erro na tela) — é adorno visual, não crítico.
- **`Avatar`** (`components/ui/`): `<img>` com a URL direto da CDN do WhatsApp (`pps.whatsapp.net`, vem pronta da Evolution API) — **sem proxy pelo backend**, a própria URL assinada já é publicamente acessível; cai pro ícone genérico (`User`/`MessagesSquare`, mesmo padrão dos ícones já usados em cliente/grupo) se não tiver foto ou a imagem falhar ao carregar (`onError`).
- **`fila`/`grupos`**: cada linha da lista virou um componente próprio (`ConversaItem`/`GrupoItem`) só pra poder chamar `useWhatsappAvatar` por linha — cada card busca sua própria foto independente das outras. Em `/grupos`, o nome vindo do WhatsApp também vira o **fallback principal** do nome exibido (`c.cliente_nome || nome || "Grupo sem nome"`), já que `cliente_nome` de grupo normalmente está vazio (não é buscado na criação, decisão inalterada — ver "Grupos do WhatsApp"). Em `/fila` só a foto é usada (nome do cliente já vem salvo).
- **Chat** (`conversas/[id]/page.tsx`): mesmo hook, avatar ao lado do nome no cabeçalho; nome do WhatsApp entra como fallback antes de "Grupo sem nome"/"Cliente sem nome".

**Custo aceito conscientemente**: cada linha de lista dispara uma chamada própria à Evolution API (N chamadas pra N conversas visíveis) — sem cache/persistência de propósito, pra manter nome/foto sempre atual sem inventar lógica de invalidação. Acredita-se aceitável pro volume esperado (poucos grupos, fila paginada em 5); se um dia isso pesar, a correção é persistir `foto_url`/nome de grupo na criação (n8n) com algum refresh periódico, não fazer cache client-side ad-hoc.

**Testado nesta sessão contra a instância real conectada** (`atendimento-empresa`, `docker ps`/logs confirmaram `CONNECTED TO WHATSAPP`) — `GET /group/findGroupInfos` e `POST /chat/fetchProfilePictureUrl` chamados direto via curl contra a Evolution API real (não só o backend) e devolveram nome/foto corretos pro grupo de teste ("Teste - Contabilidade") e foto de um cliente real. Endpoint novo do backend (`/conversations/:id/whatsapp-info`) também testado via curl contra o backend containerizado real, mesmos dois casos, resultado correto. **Não testado visualmente no navegador nesta sessão** (extensão do Chrome não conectada) — validar que os avatares realmente aparecem nas telas antes de considerar "testado com cliente real".

#### Avatar de quem escreveu cada mensagem, dentro de um grupo (mesmo dia)

Pedido em seguida: além do avatar/nome do grupo em si (seção acima), o usuário quis o avatar de **cada participante que manda mensagem dentro do grupo** — igual ao WhatsApp de verdade, onde cada balão mostra a foto de quem escreveu, não só o nome.

- **Endpoint, 1ª tentativa**: `GET /contacts/whatsapp/avatar?instance=&numero=` (independente de conversa, reaproveitando `EvolutionService.getProfilePictureUrl` direto por número). **Bug real encontrado ao testar contra o grupo de teste real**: o avatar não aparecia — `Message.remetente_telefone` (extraído de `data.key.participant` no webhook) é, em grupos com `addressingMode: "lid"` (modo de privacidade mais novo do WhatsApp — confirmado no log da Evolution API real desta sessão), um **"lid" (id vinculado)**, não o telefone de verdade. `fetchProfilePictureUrl` não acha foto buscando por um lid — confirmado comparando `messages.remetente_telefone` salvo no banco (ex: `258930810875973`) com a lista de participantes de `GET /group/findGroupInfos` (`participants[].id` = mesmo lid; `participants[].phoneNumber` = telefone real, ex: `559885159738@s.whatsapp.net`).
- **Correção**: endpoint trocado pra `GET /conversations/:id/participant-avatar?instance=&participante=` (`ConversationsController.buscarAvatarParticipante`) — resolve o lid procurando o participante na lista de `findGroupInfos` do grupo da conversa (`participants[].id.split('@')[0] === participante`) e usa `phoneNumber` (telefone real) pra buscar a foto. Se não achar (grupo sem lid, ou participante que já saiu do grupo), tenta o valor recebido direto como fallback, sem quebrar. Por isso precisa saber de qual conversa/grupo é o participante (não dá mais pra ser um endpoint solto por número, como na 1ª tentativa) — o endpoint antigo (`/contacts/whatsapp/avatar`) foi removido por ter ficado sem uso.
- **Cache client-side por telefone, desta vez sim** (diferente da decisão "sem cache" da seção acima) — dentro de `conversas/[id]/page.tsx`, um `useEffect` observa `mensagens` e, pra cada `remetente_telefone` de mensagem de grupo ainda não visto (controlado por um `Set` em `useRef`), busca a foto uma única vez e guarda num `Record<telefone, fotoUrl>` em estado. Diferença do caso anterior: aqui o mesmo participante tipicamente aparece em várias mensagens na mesma tela, então cachear evita repetir a chamada a cada mensagem dele — não é uma mudança de filosofia, é que o padrão de acesso é outro (N mensagens de poucas pessoas, não N conversas de N pessoas).
- **Render**: label acima do balão trocou de só texto pra `<Avatar size={18} .../>` + nome, mesma posição de antes.

**Testado contra o grupo real desta sessão, com o bug corrigido**: `GET /conversations/:id/participant-avatar` com um `participante` (lid) tirado direto do banco devolveu a foto de perfil correta via curl. **Não testado visualmente no navegador** — validar que aparece certo na tela, junto com o resto dos itens pendentes desta sessão.
...
Usado em vários lugares hoje: **Iniciar atendimento** (`fila/page.tsx`), **Finalizar conversa** (`conversas/[id]/page.tsx`), **Inativar** e **Excluir usuário** (`usuarios/page.tsx`, `variant="danger"` só no Excluir), **Excluir contato** (`contatos/page.tsx`). Qualquer confirmação nova no painel deve reaproveitar esse componente, não voltar pro `window.confirm`.

**Terceiro botão opcional (2026-08-17)**: `secondaryLabel`/`onSecondary`/`secondaryLoading` — quando presentes, um botão a mais aparece entre Cancelar e Confirmar (ex: "Iniciar sem mensagem" ao lado de "Iniciar com mensagem"). Os dois botões de ação (confirmar/secundário) desabilitam um ao outro enquanto qualquer um está `loading`, pra não disparar os dois ao mesmo tempo. Motivo de estender em vez de criar um modal novo: mesma filosofia de reuso já documentada acima — só esse componente lida com portal/backdrop/Escape no projeto inteiro.

### Modal ao Assumir/Finalizar — escolher se manda mensagem automática (2026-08-17)

Antes, a mensagem automática de abertura ("Olá! Tudo bem?...", ver "Respostas rápidas + mensagens automáticas" acima) e a de encerramento ("Fico à disposição!...") eram mandadas **sempre**, silenciosamente, best-effort, sem o atendente poder optar por não mandar. Pedido do usuário: perguntar antes, com a opção de pular a mensagem.

- **`fila/page.tsx`**: clicar em "Assumir" não assume mais na hora — abre `ConfirmModal` ("Iniciar atendimento?") com dois botões de ação (`confirmLabel="Iniciar com mensagem"`, `secondaryLabel="Iniciar sem mensagem"`). `handleConfirmarIniciar(comMensagem)` faz `assumeConversation` sempre, e só manda a mensagem automática (best-effort, como antes) quando `comMensagem` é `true`, antes de navegar pra `/conversas/:id`. Estado `modoIniciar: "com_mensagem" | "sem_mensagem" | null` controla qual botão mostra o spinner. O botão "Assumir" de cada linha (`ConversaItem`) perdeu o próprio spinner — quem carrega agora é o modal.
- **`conversas/[id]/page.tsx`**: mesmo padrão pro botão "Finalizar" — `ConfirmModal` existente ganhou os dois botões (`"Finalizar com mensagem"`/`"Finalizar sem mensagem"`), `handleFinalizar(comMensagem)` só manda `MENSAGEM_AUTOMATICA_FINALIZAR` quando `comMensagem`. Estado renomeado de `finalizando` (boolean) pra `modoFinalizar` (mesmo formato de três valores do `modoIniciar`).
- **Grupo não passa por isso** — grupo não tem "assumir" (ver "Grupos do WhatsApp"), e o botão Finalizar já é escondido pra `conversa.tipo === "grupo"` desde antes; nenhuma mudança nesse comportamento.

`tsc --noEmit`/`npm run build` limpos nos dois lados. **Não testado visualmente no navegador nesta sessão.**

### Rótulos de papéis editáveis — `/perfis` (só admin, 2026-08-17)

Pedido do usuário: mudar o texto exibido de cada papel no painel (ex: "Atendente" → "N1", "Supervisor" → "N2", "Administrador" → "N3", convenção de nível de suporte) sem precisar alterar código. `UserRole` (backend) continua um enum fixo (`admin`/`atendente`/`supervisor`, ver "Papel Supervisor" acima) — só o **texto mostrado** pra cada um passou a ser configurável, o papel em si não.

- **Backend**: `role_labels` — configuração global singleton (linha única), **mesmo padrão de `business_hours`** (`RoleLabelsService.obter()`/`atualizar()` espelham `BusinessHoursService` quase 1:1, incluindo o fallback em memória `ROLE_LABELS_PADRAO` se a tabela estiver vazia). `GET /role-labels` só exige login (`JwtAuthGuard`) — **todo atendente precisa ler**, o rótulo aparece no header e na tela de usuários pra todo mundo, não só admin. `PATCH /role-labels` é admin-only (`RolesGuard` + `@Roles(admin)`). `seed.ts` cria a linha padrão (`Atendente`/`Supervisor`/`Administrador`) se a tabela estiver vazia, mesmo padrão idempotente já usado pros departamentos/horário.
- **Frontend**: `useRoleLabels` (`hooks/useRoleLabels.tsx`) — `RoleLabelsProvider` montado em `(painel)/layout.tsx` ao lado de `DepartmentsProvider`/`NotificationsProvider`, busca uma vez e expõe `{ roleLabels, refresh }` pro painel inteiro. `refresh()` é chamado pela tela `/perfis` depois de salvar, pra header/tela de usuários atualizarem sem precisar de F5 — mesmo problema que motivou `NotificationsProvider` a existir como contexto global em vez de estado local por página.
- **`UserRoleLabel`** (componente local em `layout.tsx`, não em `ui/`): existe só porque `useRoleLabels()` não pode ser chamado direto em `PainelLayout` — o hook depende do `RoleLabelsProvider`, que é montado pelo próprio `PainelLayout` envolvendo esse JSX (um componente não lê um contexto que ele mesmo está prestes a criar). Precisou virar um componente filho separado, renderizado já dentro do provider.
- **`/usuarios`**: `Select` de papel e o badge da lista trocaram os literais `"Atendente"`/`"Supervisor"`/`"Administrador"` por `roleLabels.atendente`/`.supervisor`/`.admin`.
- **`/perfis`**: nova tela em `NAV_ADMIN` (dropdown "Administração"), mesmo esqueleto simples de `/horario-funcionamento` (um form, sem lista) — três campos de texto, um por papel.

**Testado nesta sessão contra o backend/frontend containerizados reais**: migration rodada, `PATCH /role-labels` chamado via curl pra já deixar os valores como `N1`/`N2`/`N3` (pedido explícito do usuário), `GET /role-labels` confirmando os três valores salvos. **Não testado visualmente no navegador** — confirmar que o header e a tela de usuários já mostram "N1"/"N2"/"N3" em vez dos nomes antigos.
...
- [x] Remetente em mensagem de grupo (`remetente_nome`/`remetente_telefone`), mensagens enviadas direto do celular conectado aparecendo no painel (`origem_externa` + dedup por `evolution_message_id`, evita duplicar o que o painel já registrou) e envio de áudio pelo painel (antes só recebia) (2026-08-17). Migration `AddMessageRemetenteEEvolutionId` rodada contra o banco de dev. `tsc`/`build` limpos nos dois lados; JSON do n8n validado (sintaxe, conexões, `jsCode` de cada node). **Workflow ainda não reimportado no n8n** (credenciais Redis/SMTP precisam ser reconfiguradas na UI de novo) — até isso acontecer, os três comportamentos não fluem de verdade num WhatsApp real. Ver "Remetente em grupo, mensagens enviadas direto do celular e envio de áudio pelo painel" acima.
...
- [x] Aba `/contatos` (aberta a todos os atendentes, não admin-only): sincroniza ao vivo os contatos do WhatsApp conectado (`GET /contacts/whatsapp`, nunca persistido) + lista própria gerenciada pelo Maré (`Contact`, migration `AddContacts`, CRUD completo, hard delete) com importar/exportar CSV feito no frontend (2026-08-17). Decisão de sincronizar do WhatsApp (em vez de lista 100% local) confirmada com o usuário antes de implementar. Ver "Aba de Contatos" acima. `tsc`/`build` limpos nos dois lados, migration rodada, `findContacts` testado contra a instância real (bug real no normalizador encontrado e corrigido nesse teste). **Não testado visualmente no navegador nesta sessão.**
- [x] Papel Supervisor (`UserRole.SUPERVISOR`, migration `AddSupervisorUserRole`) + toggle "Ver todos os setores" (`useVerTodosSetores`, 100% client-side/localStorage) em `fila`/`dashboard`, dando ao supervisor o mesmo alcance do admin quando ligado (2026-08-17). Ver "Papel Supervisor + toggle" acima. `tsc`/`build` limpos nos dois lados, migration rodada contra o banco de dev. **Não testado visualmente no navegador nesta sessão.**
- [x] Avatares (foto de perfil) na fila/grupos/chat + nome de grupo ao vivo do WhatsApp (`GET /conversations/:id/whatsapp-info`, nunca persistido) (2026-08-17). Ver "Avatares" acima. `tsc`/`build` limpos nos dois lados; testado via curl contra o backend containerizado e a Evolution API real (grupo e cliente, ambos corretos). **Não testado visualmente no navegador nesta sessão** (extensão do Chrome não conectada).
- [x] Avatar de quem escreveu cada mensagem dentro de um grupo (`GET /conversations/:id/participant-avatar`, resolve "lid" pro telefone real via `findGroupInfos`, cacheado por telefone client-side na tela de chat) (2026-08-17). Bug real (lid vs. telefone) encontrado e corrigido testando contra o grupo real. Ver "Avatar de quem escreveu cada mensagem" acima. `tsc`/`build` limpos nos dois lados. **Não testado visualmente no navegador nesta sessão.**
- [x] Modal ao Assumir/Finalizar perguntando se manda a mensagem automática ou não, em vez de mandar sempre silenciosamente (2026-08-17). `ConfirmModal` ganhou um terceiro botão opcional (`secondaryLabel`/`onSecondary`). Ver "Modal ao Assumir/Finalizar" acima. `tsc`/`build` limpos nos dois lados. **Não testado visualmente no navegador nesta sessão.**
- [x] Rótulos de papéis editáveis — `/perfis`, só admin (`role_labels`, singleton, mesmo padrão de `business_hours`) — valores já setados como `N1`/`N2`/`N3` (atendente/supervisor/admin), pedido explícito do usuário (2026-08-17). Ver "Rótulos de papéis editáveis" acima. `tsc`/`build` limpos nos dois lados, migration rodada, testado via curl. **Não testado visualmente no navegador nesta sessão.**
...
Sessão de 2026-08-17 (a mais longa até aqui): remetente em mensagem de grupo + mensagens enviadas direto do celular + envio de áudio pelo painel; gravação de áudio por microfone; aba de Contatos; papel Supervisor + toggle "ver todos os setores"; header com dropdown "Administração"; avatares (foto de perfil) + nome de grupo ao vivo do WhatsApp, incluindo o avatar de quem escreve dentro de um grupo (com correção de bug lid vs. telefone real); modal ao Assumir/Finalizar perguntando se manda mensagem automática; rótulos de papéis editáveis (`/perfis`, já configurado como N1/N2/N3). **Todo o código concluído, `tsc`/`build` limpos nos dois lados o tempo todo.** Ver "Status atual do projeto" acima pra cada item e sua seção própria mais abaixo.

Com o Docker disponível na segunda metade da sessão, boa parte das pendências de teste já foi resolvida: **todas as migrations da sessão rodaram** contra o banco de dev (`AddMessageRemetenteEEvolutionId`, `AddContacts`, `AddSupervisorUserRole`, `AddRoleLabels`), e os endpoints novos que dependem da Evolution API (`GET /contacts/whatsapp`, `GET /group/findGroupInfos`, `POST /chat/fetchProfilePictureUrl`, `GET /conversations/:id/participant-avatar`) foram testados via curl contra a instância real conectada (`atendimento-empresa`) — inclusive dois bugs reais encontrados e corrigidos nesses testes (normalizador de contatos usando `id` em vez de `remoteJid`; avatar de participante de grupo usando "lid" em vez do telefone real).

**Ainda falta, antes de considerar a sessão inteira "testada com cliente real"**: (1) reimportar `fluxo-completo-com-backend.json` no n8n e reconfigurar credenciais Redis/SMTP (mesmo trade-off de sempre) — sem isso, remetente de grupo/mensagem externa/menu dinâmico não fluem de verdade; (2) validar visualmente no navegador (extensão do Chrome não conectada em nenhum momento desta sessão) — gravação de microfone, tela de Contatos, toggle do supervisor, dropdown do header, avatares/nome de grupo aparecendo nas telas, avatar de participante numa mensagem de grupo real, os modais de Assumir/Finalizar com as duas opções (com/sem mensagem), e a tela `/perfis` mostrando/editando N1/N2/N3 no header e em `/usuarios`.
