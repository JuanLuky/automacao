...
### Mensagens de mídia do WhatsApp (imagem, áudio, documento) — receber e enviar (2026-08-05)
...
**Testado com WhatsApp real em 2026-08-05, validado pelo usuário**: imagem, áudio e documento recebidos aparecem certos no painel; anexo enviado pelo painel chegou no WhatsApp como mídia com a legenda assinada (`*Nome - Setor:*`, mesmo padrão de texto). Vídeo e figurinha confirmadamente não abrem (fora de escopo, ver acima) — aceito pelo usuário, candidato a expansão futura.
...
- [x] Mensagens de mídia do WhatsApp (imagem, áudio, documento) — receber e enviar (2026-08-05). Migration + `MediaStorageService` + endpoint de mídia no backend, detecção/download/registro no n8n (só ramo "conversa já existe"), botão de anexo + `MediaMessage` no frontend. **Testado com WhatsApp real, validado pelo usuário** — recebido e enviado funcionando; vídeo/figurinha continuam fora de escopo (não abrem, comportamento aceito). Ver "Mensagens de mídia do WhatsApp" acima, incluindo o bug do limite de body do Express (100kb → 20mb) encontrado e corrigido, e o achado pendente do `senha_hash` vazando em `Message.atendente` (não corrigido, fora de escopo dessa feature).
...
- **Corrigir `senha_hash` vazando em `Message.atendente`** — achado em 2026-08-05 durante o teste da feature de mídia (ver seção própria acima). `MessagesService.create`/`findByConversation` anexam a entidade `User` completa (via `conversa.atendente` ou `relations: ['atendente']`) sem desestruturar `senha_hash`, ao contrário do padrão já usado em `UsersService` (ver "`GET /users` e `POST /users` nunca retornam `senha_hash`" acima). Afeta a resposta HTTP de `POST/GET .../messages` **e** o payload do evento de socket `nova_mensagem` — rota é pública (sem guard), então é uma exposição real do hash bcrypt. Não corrigido ainda — fora do escopo da feature de mídia, mas vale priorizar.
- **Vídeo e figurinha (sticker) do WhatsApp** — fora de escopo deliberado da feature de mídia (2026-08-05, ver seção acima): continuam caindo como mensagem de texto vazia, sem abrir no painel. Confirmado com o usuário que é aceitável por ora. Expandir seguindo o mesmo padrão já estabelecido (tipo no enum, allowlist de mimetype, detecção no `Extrair Dados da Mensagem`, renderização em `MediaMessage.tsx`) se virar necessidade real.
