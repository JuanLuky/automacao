...
- **Áudio do painel sempre em ogg/opus antes de ir pra Evolution API**: `MediaStorageService.normalizarAudioParaWhatsapp` chama o binário `ffmpeg` do sistema (não é dependência npm) pra reencodificar qualquer áudio que o atendente manda — o `MediaRecorder` do navegador grava webm no Chrome/Edge, e o WhatsApp só reproduz nota de voz em ogg/opus; sem isso a Evolution API aceita o upload (2xx) mas o WhatsApp descarta a mensagem sem avisar. Exige `ffmpeg` instalado onde o backend rodar (já vem na imagem Docker; rodando nativo, instalar à parte — ver `SETUP-NOVA-MAQUINA.md`).
...
- **Rotas chamadas pelo n8n sem login de atendente**: `GET /conversations/by-phone/:telefone`, `POST /conversations`, `POST /conversations/:id/messages`, `GET /departments`, `GET /business-hours`, `POST /bot-sessions/:telefone/mensagem-enviada`, `PATCH`/`DELETE /messages/evolution/:id` — protegidas por `N8nOrJwtAuthGuard` (`backend/src/auth/guards/n8n-or-jwt-auth.guard.ts`), que aceita **ou** um JWT do painel **ou** o header `x-n8n-api-key` batendo com `N8N_API_KEY` (comparação com `timingSafeEqual`). `GET /status/*` continua público de verdade (página de status pública, sem relação com o n8n). Antes disso (até 2026-09-04) essas rotas eram públicas sem nenhuma credencial — reforçado a pedido do usuário como pré-requisito pra vender o sistema.
...
- **Mídia enviada antes da escolha do setor** (ramo "sem conversa ativa") passa por um ramo próprio — `É Mídia Sem Conversa?` → `Buscar Mídia (Bot)` → `Montar Mídia do Bot` → `POST /bot-sessions/:telefone/midia-recebida` — espelhando o ramo já existente pra conversa já criada (`É Mensagem de Mídia?` → `Buscar Mídia` → `Montar Mensagem de Mídia`). Sem isso, imagem/PDF/áudio mandado antes de escolher o setor nunca era salvo em lugar nenhum (nem arquivo, nem menção no histórico) — perda permanente, não um filtro de exibição. `BotSessionsService.registrarMidia` grava o arquivo (mesmo `MediaStorageService` das mensagens normais) e some do histórico do bot pra virar `Message` de verdade assim que a conversa nasce (`ConversationsService.inserirHistoricoBot`).
