...
Sessão de 2026-08-17 (a mais longa até aqui): remetente em mensagem de grupo + mensagens enviadas direto do celular + envio de áudio pelo painel; gravação de áudio por microfone; aba de Contatos; papel Supervisor + toggle "ver todos os setores"; header com dropdown "Administração"; avatares (foto de perfil) + nome de grupo ao vivo do WhatsApp, incluindo o avatar de quem escreve dentro de um grupo; modal ao Assumir/Finalizar perguntando se manda mensagem automática. **Todo o código concluído, `tsc`/`build` limpos nos dois lados o tempo todo.** Ver "Status atual do projeto" acima pra cada item e sua seção própria mais abaixo.

Com o Docker disponível na segunda metade da sessão, boa parte das pendências de teste já foi resolvida: **todas as migrations da sessão rodaram** contra o banco de dev (`AddMessageRemetenteEEvolutionId`, `AddContacts`, `AddSupervisorUserRole`), e os endpoints novos que dependem da Evolution API (`GET /contacts/whatsapp`, `GET /group/findGroupInfos`, `POST /chat/fetchProfilePictureUrl`) foram testados via curl contra a instância real conectada (`atendimento-empresa`) — inclusive um bug real encontrado e corrigido no normalizador de contatos (`id` não é o JID, é a chave interna do registro; o campo certo é `remoteJid`).

**Ainda falta, antes de considerar a sessão inteira "testada com cliente real"**: (1) reimportar `fluxo-completo-com-backend.json` no n8n e reconfigurar credenciais Redis/SMTP (mesmo trade-off de sempre) — sem isso, remetente de grupo/mensagem externa/menu dinâmico não fluem de verdade; (2) validar visualmente no navegador (extensão do Chrome não conectada em nenhum momento desta sessão) — gravação de microfone, tela de Contatos, toggle do supervisor, dropdown do header, avatares/nome de grupo aparecendo nas telas, avatar de participante numa mensagem de grupo real, e os modais de Assumir/Finalizar com as duas opções (com/sem mensagem).
